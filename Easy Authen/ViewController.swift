...
class ViewController: UIViewController {
    ...
    func checkSpace(strUser:String, strPass:String) -> Void {
        if ((strUser.isEmpty) || (strPass.isEmpty)) {
            ...
            myAlertDialog("มีช่องว่าง", strMessage: "กรุณากรอกทุกช่องคะ")
            
            
        }  else {
            
            var trueUswer = ["test1", "test2", "test3"]
            var truePass = ["123", "456", "789"]
            var index = 0
            var status = false
            var myTruePass = ""
            
            for myfor in trueUswer {
...
//                    self.navigationController?.pushViewController(goToService, animated: true)
                    ...
                    myAlertDialog("Password False", strMessage: "Please Try Again Passoword False")
